...
#pragma mark - AES加密
//将string转成带密码的data
+(NSData*)encryptAESData:(NSString*)string publick:(NSString *)pulickey {
    //将nsstring转化为nsdata
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    //使用密码对nsdata进行加密
    NSData *encryptedData = [data AES256EncryptWithKey:pulickey];
    return encryptedData;
}

//将带密码的data转成string
+(NSString*)decryptAESData:(NSData*)data publick:(NSString *)pulickey{
    //使用密码对data进行解密
    NSData *decryData = [data AES256DecryptWithKey:pulickey];
