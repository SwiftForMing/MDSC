...
@interface GCDFireSocketManger()<GCDAsyncSocketDelegate>{
...
@implementation GCDFireSocketManger

- (void)dealloc
{
    MLog(@"GCDFireSocketManger %@", NSStringFromSelector(_cmd));
}
...
-(void)loginCheckWithData:(NSData *)data
{
    ...
    if (moudel == 6&&cmd==4) {
        if (code == 0) {
            MLog(@"=====弹幕返回成功");
//            [self checkJasonDataWithData:jsonData code:105];
