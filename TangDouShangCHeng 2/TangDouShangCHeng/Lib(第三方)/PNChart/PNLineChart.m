...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, y, (NSInteger) _chartMarginLeft, (NSInteger) _yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = (index * _xLabelWidth + 2*_chartMarginLeft);
//              NSInteger x = ((index+1) * _xLabelWidth + _chartMarginLeft + _xLabelWidth/4.0 );
            NSInteger y = _chartMarginBottom + _chartCavanHeight;

            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(x, y, (NSInteger) _xLabelWidth/2, (NSInteger) _chartMarginBottom)];
            ...
            label.transform = CGAffineTransformMakeRotation(-(45%360)*(M_PI/180));
            if (index%2!=0) {
                label.hidden = true;
            }
...
//            int x = i * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0;
            int x = i * _xLabelWidth + _chartMarginLeft*2;
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                [pointPath moveToPoint:CGPointMake(circleCenter.x + (inflexionWidth/4), circleCenter.y)];
                [pointPath addArcWithCenter:circleCenter radius:inflexionWidth/4 startAngle:0 endAngle:2 * M_PI clockwise:YES];
...
- (void)drawRect:(CGRect)rect {
    if (self.isShowCoordinateAxis) {
        ...
        if ([self.yUnit length]) {
            CGFloat height = [PNLineChart sizeOfString:self.yUnit withWidth:40.f font:font].height;
            CGRect drawRect = CGRectMake(0, 0, 40.f, height);
            [self drawTextInContext:ctx text:self.yUnit inRect:drawRect font:font];
        }

        // draw x unit
        if ([self.xUnit length]) {
            CGFloat height = [PNLineChart sizeOfString:self.xUnit withWidth:40.f font:font].height;
            CGRect drawRect = CGRectMake(CGRectGetWidth(rect) - _chartMarginLeft + 5, _chartMarginBottom + _chartCavanHeight, 25.f, height);
    ...
    if (1) {
        ...
        CGPoint point;
        point = CGPointMake(_chartMarginLeft + yAxisOffset,_nowY);
        CGContextMoveToPoint(ctx, point.x, point.y);
        CGPoint point1;
        point1 = CGPointMake(_chartMarginLeft + yAxisOffset,_zeroY);
        CGPoint point2;
        point2 = CGPointMake(_chartMarginLeft + yAxisOffset,_buyY);
        //自己要买的
        CGContextRef contextRef = UIGraphicsGetCurrentContext();
        CGColorSpaceRef rgb = CGColorSpaceCreateDeviceRGB();
        CGFloat colors[] =
        {
            226/255.0,102/255.0,80/255.0,0.3,
            226/255.0,102/255.0,80/255.0,0.3,
        };
        ...
        CGContextMoveToPoint(contextRef, point2.x, point2.y-1);
        CGContextAddLineToPoint(contextRef, CGRectGetWidth(rect) - _chartMarginLeft/2, point2.y);
        CGContextAddLineToPoint(contextRef, CGRectGetWidth(rect) - _chartMarginLeft/2, point.y);
        CGContextAddLineToPoint(contextRef, point.x, point.y-1);
        CGContextClip(contextRef);//context裁剪路径,后续操作的路径
        //CGContextDrawLinearGradient(CGContextRef context,CGGradientRef gradient, CGPoint startPoint, CGPoint endPoint,CGGradientDrawingOptions options)
        //gradient渐变颜色,startPoint开始渐变的起始位置,endPoint结束坐标,options开始坐标之前or开始之后开始渐变
        CGContextDrawLinearGradient(contextRef, gradient,CGPointMake(point2.x, point2.y-1) ,CGPointMake(point.x, point.y-1),kCGGradientDrawsAfterEndLocation);
        CGContextRestoreGState(contextRef);// 恢复到之前的c
        
        //已经买了的
        CGContextRef contextRef1 = UIGraphicsGetCurrentContext();
        CGColorSpaceRef rgb1 = CGColorSpaceCreateDeviceRGB();
        CGFloat colors1[] =
        {
           161/255.0,196/255.0,254/255.0,1,
             1,1,1, 1.00,
        };
        CGGradientRef gradient1 = CGGradientCreateWithColorComponents
        (rgb1, colors1, NULL, sizeof(colors1)/(sizeof(colors1[0])*4));//形成梯形，渐变的效果
        CGColorSpaceRelease(rgb);
        CGContextSaveGState(contextRef1);
        CGContextMoveToPoint(contextRef1, point.x, point.y-1);
        CGContextAddLineToPoint(contextRef1, CGRectGetWidth(rect) - _chartMarginLeft/2, point.y);
        CGContextAddLineToPoint(contextRef1, CGRectGetWidth(rect) - _chartMarginLeft/2, point1.y);
        CGContextAddLineToPoint(contextRef1, point1.x, point1.y-1);
        CGContextClip(contextRef1);//context裁剪路径,后续操作的路径
        //CGContextDrawLinearGradient(CGContextRef context,CGGradientRef gradient, CGPoint startPoint, CGPoint endPoint,CGGradientDrawingOptions options)
        //gradient渐变颜色,startPoint开始渐变的起始位置,endPoint结束坐标,options开始坐标之前or开始之后开始渐变
        CGContextDrawLinearGradient(contextRef1, gradient1,CGPointMake(point.x, point.y-1) ,CGPointMake(point1.x, point1.y-1),kCGGradientDrawsAfterEndLocation);
        CGContextRestoreGState(contextRef1);// 恢复到之前的
...
//        CGFloat xAxisOffset = _showLabel ? 10.f : 0.0f;
        CGPoint point;
        CGFloat xStepWidth = _xLabelWidth;
        if (self.yGridLinesColor) {
            CGContextSetStrokeColorWithColor(ctx, self.yGridLinesColor.CGColor);
        } else {
            CGContextSetStrokeColorWithColor(ctx, [UIColor lightGrayColor].CGColor);
        ...
        for (NSUInteger i = 0; i < _xLabels.count+1; i++) {
             point = CGPointMake(((_xLabels.count*xStepWidth)+_chartMarginLeft+_chartMarginRight - i * xStepWidth), _chartMarginTop-3);
            CGContextMoveToPoint(ctx, point.x, point.y);
            // add dotted style grid
            CGFloat dash[] = {5, 0};
            // dot diameter is 20 points
            ...
            CGContextSetLineCap(ctx, kCGLineCapRound);
            CGContextSetLineDash(ctx, 0.0, dash, 2);
            CGContextAddLineToPoint(ctx, point.x,CGRectGetHeight(rect)-xStepWidth+_chartMarginBottom/2);
            CGContextStrokePath(ctx);
        }
    }
...
-(UIColor *)getColorWithSize:(CGSize)size startColor:(UIColor *)startColor endColor:(UIColor *)endColor{
    //创建CGContextRef
    ...
    UIGraphicsEndImageContext();
     ...
     return [[UIColor alloc]initWithPatternImage:img];
...
//    CGPoint startPoint = CGPointMake(0, 0);
    CGPoint startPoint = CGPointMake(0, CGRectGetMaxY(pathRect));
...
- (CATextLayer *)createPointLabelFor:(CGFloat)grade pointCenter:(CGPoint)pointCenter width:(CGFloat)width withChartData:(PNLineChartData *)chartData {
    ...
    [textLayer setCornerRadius:textLayer.fontSize];
    ...
    UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake((textWidth-textHeight)/2, 0, textHeight, textHeight)];
   
    if (_isFire){
        if (grade == 1){
            imageView.image = [UIImage imageNamed:@"fireBgua"];
            //        [textLayer setString:[[NSString alloc] initWithFormat:_yLabelFormat, "grade"]];
        }
        if (grade == 2){
            imageView.image = [UIImage imageNamed:@"jingWord"];
            [textLayer setString:[[NSString alloc] initWithFormat: @"金"]];
        }
        if (grade == 3){
            imageView.image = [UIImage imageNamed:@"muWord"];
            [textLayer setString:[[NSString alloc] initWithFormat:@"木"]];
        }
        if (grade == 4){
            imageView.image = [UIImage imageNamed:@"tuWord"];
            [textLayer setString:[[NSString alloc] initWithFormat:@"土"]];
        }
        if (grade == 5){
            imageView.image = [UIImage imageNamed:@"waterWord"];
            [textLayer setString:[[NSString alloc] initWithFormat:@"水"]];
        }
        if (grade == 6){
            imageView.image = [UIImage imageNamed:@"fireWord"];
            [textLayer setString:[[NSString alloc] initWithFormat:@"火"]];
        }
//       [textLayer addSublayer:imageView.layer];
    }else{
