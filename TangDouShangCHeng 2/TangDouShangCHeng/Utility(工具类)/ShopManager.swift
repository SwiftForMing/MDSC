...
class ShopManager: NSObject {
    ...
    static let shareManager = ShopManager()
    ...
    override init(){
        ...
        let path = NSHomeDirectory() + "/Documents/fmdbshopcar.sqlite"
        //2.构造管理数据库的对象
...
//            print("1234567890")
    ...
    func insertDataWith(_ model:HomeGoodModel) {
        ...
        let insertSql = "insert into data(id,good_imgs,coupons_price,good_price,coupons_id,profit_price,goods_type_id,good_header,good_name,order_num,coupons_name,valid_date,after_coupons_price,coupons_value,coupons_type,coupons_condition) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        ...
        do{
            ...
            Tool.showPromptContent("加入购物车成功")
        }catch{
    ...
    func deleteDataWith(model:HomeGoodModel) {
        lock.lock()
        let deleteSql = "delete from data where id = ?"
        //更新数据库
        do{
    ...
    func updateDataWith(model:HomeGoodModel,dataID:String) {
        ...
        let updateSql = "update data set id = ?,good_imgs = ?,coupons_price = ?,good_price = ?,coupons_id = ?,profit_price = ?,goods_type_id = ?,good_header = ?,good_name = ?,order_num = ?,coupons_name = ?,valid_date = ?,after_coupons_price = ?,coupons_value = ?,coupons_type = ?,coupons_condition = ? where id = ?"
        //         let updateSql = "update data set id = ?,good_imgs = ? where id = ?"
        //更新数据库
        do{
    ...
    func isHasDataInTable(model:HomeGoodModel)->Bool {
        ...
        let isHasSql = "select * from data where id = ?"
        do{
